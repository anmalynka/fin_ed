...
function App() {
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-20 animate-in fade-in zoom-in duration-700">
      <div className="relative mb-8">
        ...
        <img src={heroImg} alt="Financial Hero" className="relative w-48 h-48 object-contain" />
      </div>
      <h2 className="text-3xl font-black text-slate-800 tracking-tight mb-2 text-center uppercase">Institutional Intelligence</h2>
      <p className="text-slate-400 font-bold text-center max-w-sm">Enter a stock or ETF ticker to unlock Peter Lynch Fair Value and AI forecasts.</p>
      <div className="mt-8 flex flex-wrap justify-center gap-3">
        {['AAPL', 'NVDA', 'TSLA', 'SPY', 'XOM', 'SCHD'].map(t => (
          <button key={t} onClick={() => { setTicker(t); }} className="px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black uppercase text-slate-500 hover:bg-white hover:shadow-sm transition-all">{t}</button>
        ))}
      </div>
    </div>
  );
  ...
  const NotFoundState = () => (
    <div className="flex flex-col items-center justify-center py-20 animate-in slide-in-from-bottom-10 duration-500">
      ...
      <h2 className="text-3xl font-black text-slate-800 tracking-tight mb-2 uppercase italic text-center">Analysis Failed</h2>
      <p className="text-slate-400 font-bold text-center max-w-sm px-6">{errorMessage}</p>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  {mode === 'perf' ? (
                    <LineChart data={history}>
                      ...
                      {zones && (
                        <>
                          <ReferenceArea y1={zones.resistance?.low} y2={zones.resistance?.high} fill="#fee2e2" fillOpacity={0.6} />
                          <ReferenceArea y1={zones.support?.low} y2={zones.support?.high} fill="#d1fae5" fillOpacity={0.6} />
                        </>
                      )}
